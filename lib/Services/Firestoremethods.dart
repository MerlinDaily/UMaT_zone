...
class FirestoreMethods {
  ...
  //Upload post
  Future<String> Uploadpost(String Title, String Details, dynamic file,
      String uid, String author, String ppurl) async {
    String photourl;
    String err;
    try {
      if (file != null) {
  ...
  Future<void> likepost(String postid, String author_uid, List likes,
      String owneruid, String author, String ppurl, String title) async {
    try {
      if (likes.contains(author_uid)) {
        ...
        await _firestore
            ...
            .collection("Notifs")
            .doc(notifid)
            .set(nots.toJson(), SetOptions(merge: true));
      }
    } catch (e) {
      print(e.toString());
    }
  ...
  Future<String> postcomment(String postid, String text, String author_uid,
      String author, String ppurl, String title, String owner_uid) async {
    String ress;
    try {
      if (text.isNotEmpty) {
        ...
        await _firestore
            ...
            .collection("comments")
            .doc(commentid)
            .set(comments.toJson(), SetOptions(merge: true));
        ...
        await _firestore
            ...
            .collection("Notifs")
            .doc(notifid)
            .set(notifs.toJson(), SetOptions(merge: true));
...
//Edit post
  Future<String> Editpost(String postid, String? title, String? details,
      dynamic image, bool upload) async {
    String ress;
    String photourl;
    try {
      if (image != null && upload) {
        ...
        if (title != "") {
          await _firestore
              .collection("Posts")
              .doc(postid)
              .update({'title': title, 'searchkey': title!.substring(0, 1)});
        }
      } else {
        photourl = "";
        await _firestore.collection("Posts").doc(postid).update({
...
//editcomment
  Future<String> Editcomment(
      String postid, String commentid, String details) async {
    String ress;
    try {
      await _firestore
          .collection("Posts")
          ...
          .collection('comments')
          .doc(commentid)
          .update({'detail': details, 'Comment Time': DateTime.now()});
  ...
  Future<String> EditProfile(
    ...
    try {
      ...
      if (image != null) {
        ppurl =
            await StorageMethods().Storageip("Profilepic", image, false, null);
      } else {
        ppurl = Profileurl;
      }
  ...
  Future<void> Updatecommentpic(
      String postid, String ppurl, String commentid) async {
    await _firestore
        .collection("Posts")
        .doc(postid)
        .collection("comments")
...
//Receiver Side
      ...
      await _firestore
          ...
          .collection("message")
          .doc(message_uid)
          .set(chatr.toJson(), SetOptions(merge: true));
      await _firestore
          .collection("Chats")
          .doc(receiver_uid)
          .collection("Chathead")
  ...
  Future<String> CreateGroup(String uid, String author, String grpname,
      String grpdesc, dynamic file, User1 user) async {
    String ress;
    String photourl;
    List members = [];
    ...
    try {
      if (file != null) {
        photourl = await StorageMethods()
            .Storageip("Group Profile", file, true, groupid);
      } else {
        photourl = "";
      }
      members.add(uid);
      ...
      Group group = Group(
          ...
          members: members,
          Group_name:
              grpname.substring(0, 1).toUpperCase() + grpname.substring(1),
  ...
  Future<String> UpdateHeader(String Groupid, dynamic file) async {
    ...
    try {
      if (file != null) {
        photourl = await StorageMethods()
            .Storageip("Group Header", file, true, Groupid);
      } else {
        photourl = "";
      }
  ...
  Future<String> GroupPost(String Groupid, String uid, String Title,
      String Detail, dynamic file, String author, String ppurl) async {
    String ress;
    String photourl;
    try {
      if (file != null) {
        photourl =
            await StorageMethods().Storageip("GroupPosts", file, true, Groupid);
      } else {
        photourl = "";
      }
      String postid = const Uuid().v1();
      ...
      _firestore
          ...
          .collection("Posts")
          .doc(postid)
          .set(gpost.toJson(), SetOptions(merge: true));
      _firestore.collection("Groups").doc(Groupid).update({
  ...
  Future likedgroupost(
      String postid, String author_uid, List likes, String Groupid) async {
    try {
      if (likes.contains(author_uid)) {
        await _firestore
            .collection("Groups")
  ...
  Future<String> UpdateGroup(String uid, String Groupname, String Groupdesc,
      String memberalias, String postalias, dynamic file) async {
    String ress;
    String photourl;
    ...
    try {
      if (file != null) {
        photourl =
            await StorageMethods().Storageip("Group Profile", file, true, uid);
      } else {
        photourl = "";
      }
      if (photourl != "") {
  ...
  Future<String> Groupcomment(String groupid, String postid, String text,
      String author_uid, String author, String ppurl) async {
    String ress;
    try {
      if (text.isNotEmpty) {
        String commentid = const Uuid().v1();
  ...
  Future<String> EditGroupPost(String groupid, String postid, String title,
      String details, dynamic image, bool upload) async {
    String ress;
    String photourl;
    try {
      if (image != null && upload) {
        photourl =
            await StorageMethods().Storageip("GroupPosts", image, true, null);
        await _firestore
            .collection("Groups")
            .doc(groupid)
            .collection("Posts")
  ...
  Future<String> Deletegroupcomment(
  ...
  Future<String> Editgroupcomment(
      String groupid, String postid, String commentid, String text) async {
    String ress;
    try {
      await _firestore
          .collection("Groups")
  ...
  Future<String> Groupchat(String groupid, String author_uid, String message,
      String ppurl, String author) async {
    String ress;
    try {
      ...
      await _firestore
          ...
          .collection("Chats")
          .doc(message_uid)
          .set(groupchat1.toJson(), SetOptions(merge: true));
  ...
  Future<String?> UpdateThemeMode(String uid, bool Mode) async {
    ...
    try {
      if (Mode == true) {
        //Dark Theme
        UserThemeData themedata = UserThemeData(UID: uid, DarkMode: true);
        await _firestore
            .collection("Settings")
            .doc(uid)
            .collection("Theme")
            .doc(uid)
            .set(themedata.toJson(), SetOptions(merge: true));
      } else {
